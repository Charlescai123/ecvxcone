...
class ECVXCONEInterface(SolverInterface):
    ...
    solver_type = 'conic'
    canon_p_ids = ['c', 'A', 'b', 'G', 'h', 'd']
    canon_p_ids_constr_vec = ['b', 'h']
    supports_gradient = False
    solve_function_call = '{prefix}ecvxcone_flag = conelp({prefix}ecvxcone_ws, &ecvxcone_settings)'

    # header files and sources
    header_files = ['"cvxopt.h"', '"misc.h"', '"solver.h"']
    cmake_headers = ['${ecvxcone_headers}']
    cmake_sources = ['${ecvxcone_sources}']

    inmemory_preconditioning = True
    ws_statically_allocated_in_solver_code = False

    ws_ptrs = WorkspacePointerInfo(
        objective_value = 'ecvxcone_ws->result->primal_objective',
        iterations = 'ecvxcone_ws->result->iterations',
        status = 'ecvxcone_ws->result->status',
        primal_residual = 'ecvxcone_ws->result->primal_infeasibility',
        dual_residual = 'ecvxcone_ws->result->dual_infeasibility',
        primal_solution = 'MAT_BUFD(ecvxcone_ws->result->x)',
        dual_solution = 'MAT_BUFD(ecvxcone_ws->result->z)',
        settings = 'ecvxcone_settings'
    )
    ...
    def __init__(self, data, p_prob, enable_settings):
        ...
        G_nnz = 0 if data['G'] is None else data['G'].nnz
        A_nnz = 0 if data['A'] is None else data['A'].nnz
    ...
    def declare_workspace(self, f, prefix, parameter_canon) -> None:
        ...
        # Declare ECVXCONE settings
        f.write(f'\n// ECVXCONE Default settings\n')
        write_struct_prot(f, f'{prefix}ecvxcone_settings', 'ECVXConeSettings')
        
        # Declare ECVXCONE Cone Dims
        f.write(f'\n// ECVXCONE Cone dimensions\n')
        write_struct_prot(f, f'{prefix}ecvxcone_dims', 'DIMs')
        
        # Declare ECVXCONE workspace
        f.write(f'\n// ECVXCONE workspace\n')
        write_struct_prot(f, f'{prefix}ecvxcone_ws', 'ECVXConeWorkspace*')
        
        # Declare core solver function
        f.write(f'\n// ECVXCONE solver function\n')
        f.write(f'extern int conelp(ECVXConeWorkspace* ecvxcone_ws, ECVXConeSettings* settings);\n')

        # Declare ECVXCONE setup function
        f.write(f'\n// ECVXCONE setup function\n')
        f.write(f'extern ECVXConeWorkspace* ecvxcone_setup(int n_var, int n_ineq, int n_eq, int nnz_G, int nnz_A, DIMs *dims, ECVXConeSettings *settings);\n')

        # Declare ECVXCONE exit flag
        f.write(f'\n// ECVXCONE exit flag\n')
        f.write(f'extern cpg_int {prefix}ecvxcone_flag;\n')
    ...
    def define_workspace(self, f, prefix, parameter_canon) -> None:
        ...
        if self.canon_constants['q_size'] > 0:  # SOC buf
            write_vec_def(f, self.canon_constants['q'], f'{prefix}q_buf', 'int')
            f.write('\n')
        if self.canon_constants['s_size'] > 0:  # PSD buf
            write_vec_def(f, self.canon_constants['s'], f'{prefix}s_buf', 'int')
            f.write('\n')
        ...
        write_struct_def(f, cone_dims_fields, cone_dims_casts, cone_dims_values, f'{prefix}ecvxcone_dims', 'DIMs')

        # Define the ECVXCONE workspace
        f.write('\n// ECVXCONE workspace\n')
        f.write(f'ECVXConeWorkspace* {prefix}ecvxcone_ws = NULL;\n')
        f.write('\n// ECVXCONE exit flag\n')
        f.write(f'cpg_int {prefix}ecvxcone_flag = -1;\n')
