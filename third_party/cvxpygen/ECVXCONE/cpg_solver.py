...
from ECVXCONE import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["A", "B", "tracking_err_square"]
    ...
    par.tracking_err_square = list(param_dict["tracking_err_square"].value.flatten(order="F"))
    ...
    prob.var_dict['Q'].save_value(np.array(res.cpg_prim.Q).reshape((10, 10), order='F'))
    prob.var_dict['R'].save_value(np.array(res.cpg_prim.R).reshape((6, 10), order='F'))
    prob.var_dict['T'].save_value(np.array(res.cpg_prim.T).reshape((6, 6), order='F'))
    prob.constraints[0].save_dual_value(np.array(res.cpg_dual.d0).reshape((20, 20), order='F'))
    prob.constraints[1].save_dual_value(np.array(res.cpg_dual.d1).reshape((16, 16), order='F'))
    prob.constraints[2].save_dual_value(np.array(res.cpg_dual.d2).reshape((10, 10), order='F'))
    prob.constraints[3].save_dual_value(np.array(res.cpg_dual.d3).reshape((3, 3), order='F'))
    prob.constraints[4].save_dual_value(np.array(res.cpg_dual.d4).reshape((6, 6), order='F'))
    prob.constraints[5].save_dual_value(np.array(res.cpg_dual.d5).reshape((6, 6), order='F'))

    # store additional solver information in problem object
    prob._status = "%d (for description visit https://cvxopt.org/userguide/coneprog.html)" % res.cpg_info.status
    ...
    results_dict = {'solver_specific_stats': solver_specific_stats,
                    ...
                    'solve_time': t1 - t0}
    prob._solver_stats = SolverStats.from_dict(results_dict, 'ECVXCONE')
